# tools.py — Mārjak v2: LLM Navigates, Python Protects
#
...
def navigate(path: str) -> str:
    ...
    # Dual-write to persistent store
    top_children = [
        {"name": e["name"], "size": e.get("size", 0), "is_dir": e.get("is_dir", False)}
        for e in sorted(entries, key=lambda x: x.get("size", 0), reverse=True)[:10]
    ]
    fs_memory.upsert_directory(
        path=expanded, name=os.path.basename(expanded) or expanded,
        size_bytes=total_size, item_count=len(entries),
        top_children=top_children,
    )
    fs_memory.log_action("navigate", expanded, f"{_human_size(total_size)}, {len(entries)} items")

    # Record in persistent memory (legacy — will be removed)
...
def mole_scan() -> str:
    ...
    try:
        ...
        for line in process.stdout:
            ...
            # Parse for session playbook updates
    ...
    fs_memory.log_action("mole_scan", "", f"Previewed {cats_found} cleanable categories")
    console.print("[bold red]✔ Mole scan complete (no changes made).[/bold red]\n")
    return f"Mole scan preview complete. Added {cats_found} auto-cleanable categories to the session playbook."
...
def search_system(name: str, file_type: str = "any") -> str:
    ...
    # Dual-write search hits to persistent store
    for item in enriched[:10]:
        if item.get("is_dir"):
            fs_memory.upsert_search_hit(item["path"], name, item.get("size", 0))
    fs_memory.log_action("search_system", "", f"'{name}': {len(enriched)} results")
...
def move_to_trash(file_ids: list[int]) -> str:
    ...
    Provide the exact integer File IDs [FID: X] from the explored tree.
    ...
    # Deduplicate: if a parent directory is in the list, skip all its children
    # (moving the parent already moves everything inside it)
    deduplicated = []
    valid_sorted = sorted(valid_targets, key=len)  # shortest paths first
    ...
    skipped = len(valid_targets) - len(deduplicated)
    if skipped:
        results.append(f"ℹ Skipped {skipped} children already covered by parent directories.")
    valid_targets = deduplicated
    ...
    for absolute in valid_targets:
        try:
            ...
            # Fallback: use osascript (handles extended attrs, SIP-adjacent files)
            try:
                ...
                results.append(f"✔ Moved to Trash (via Finder): {absolute}")
                session_book.remove_node(absolute)
            except subprocess.CalledProcessError as e2:
                results.append(f"ERROR: Failed to move '{absolute}': {e2}")

    report = "\n".join(results)
    memory.record_action("move_to_trash", report[:120])
    # Dual-delete from persistent store
    for absolute in valid_targets:
        fs_memory.delete_path(absolute)
    fs_memory.log_action("move_to_trash", "", report[:120])
    return report
